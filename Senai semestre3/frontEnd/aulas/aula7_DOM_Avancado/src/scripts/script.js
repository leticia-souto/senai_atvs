...
function adicionarItemAoResumo(nome, qtd, preco, cardOrigem) {
  ...
  //é aqui que é realmente inserido na página (parte visual)
...
if (btnLimpar) {
  btnLimpar.addEventListener("click", () => {
    ...
    // Remove todos os badges dos cards que criamos no JS (não tem no HTML)
    document.querySelectorAll(".badge-adicionado").forEach((b) => b.remove());

    // Remove filhos da lista um a um com firstElementChild (remove todos os itens, mesma função do de cima)
    while (listaResumo.firstElementChild) {
      listaResumo.firstElementChild.remove();
    }

    //faz os itens sumires, não são apagados apenas somem
    secaoResumo.style.display = "none";
  });
}