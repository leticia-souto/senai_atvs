...
function salvarJogo(jogo) {
  ...
  //localStorage só guarda texto, JSON.stringify converte p formato válido antes de salvar
...
function renderizarJogos() {
  ...
  //lista.length pega a qtd de jogos total da lista
  contador.textContent = `Você tem ${lista.length} jogos na sua lista`
  ul.innerHTML = "";
  ...
  lista.forEach(function (jogo, indice) {
    ...
    li.innerHTML = `
      ...
      <div class="meta">Produtora: ${jogo.produtora} • Nota: ${jogo.nota}/5 • Plataforma: ${jogo.plataforma}</div>
...
document.querySelector("#lista-jogos").addEventListener("click", function (event) {
  ...
  //OBS: tem que usar class, o id  esta vazio
